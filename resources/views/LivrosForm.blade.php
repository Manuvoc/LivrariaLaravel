    ...
    <div class="container">
        ...
        <form action='{{ $route }}' method="POST" enctype="multipart/form-data">
            ...
            <input type="hidden" name="id"
                value="@if (!empty(old('id'))) {{ old('id') }} @elseif(!empty($livros->id)) {{ $livros->id }} @else {{ '' }} @endif" /><br>
            <div class="col-3">
                <label class="form-label">Nome do Livro</label><br>
                <input type="text" class="form-control" name="nome"
                    value="@if (!empty(old('nome'))) {{ old('nome') }} @elseif(!empty($livros->nome)) {{ $livros->nome }} @else {{ '' }} @endif" /><br>
            </div>
            <div class="col-3">
                <label class="form-label">Valor</label><br>
                <input type="text" class="form-control" name="valor"
                    value="@if (!empty(old('valor'))) {{ old('valor') }} @elseif(!empty($livros->valor)) {{ $livros->valor }} @else {{ '' }} @endif" /><br>
            </div>
            <div class="col-3">
                <label class="form-label">Genero dos Livros</label><br>
                <select name="generolivro_id" class="form-select">
                    @foreach ($generolivros as $item)
            ...
            <a href='{{ route('livros.index') }}' class="btn btn-primary"><i class="fa-solid fa-arrow-left"></i>
                Voltar</a>
        </form>
    </div>
    <!-- Footer-->
