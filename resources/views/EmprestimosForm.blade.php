...
@php
    if (!empty($emprestimo->id)) {
        $route = route('emprestimo.update', $emprestimo->id);
    } else {
        $route = route('emprestimo.store');
    }
@endphp


    <div class="container">
        <h1>Formulário Emprestimos</h1>
        ...
        <form action='{{ $route }}' method="POST" enctype="multipart/form-data">
            ...
            <input type="hidden" name="id"
                value="@if (!empty(old('id'))) {{ old('id') }} @elseif(!empty($emprestimo->id)) {{ $emprestimo->id }} @else {{ '' }} @endif" /><br>
            <div class="col-3">
            ...
            <br>
            <div class="col-3">
                <label class="form-label">Data Retirada</label><br>
                <input type="date" class="form-control" name="dataretirada"
                    value="@if (!empty(old('dataretirada'))) {{ old('dataretirada') }} @elseif(!empty($emprestimo->dataretirada)) {{ $emprestimo->dataretirada }} @else {{ '' }} @endif" /><br>
            </div>
            <div class="col-3">
                <label class="form-label">Data Devolução</label><br>
                <input type="date" class="form-control" name="datadevolucao"
                    value="@if (!empty(old('datadevolucao'))) {{ old('datadevolucao') }} @elseif(!empty($emprestimo->datadevolucao)) {{ $emprestimo->datadevolucao }} @else {{ '' }} @endif" /><br>
            </div>

            <button class="btn btn-success" type="submit">
                <i class="fa-solid fa-save"></i> Salvar
            </button>
            <a href='{{ route('emprestimo.index') }}' class="btn btn-primary"><i class="fa-solid fa-arrow-left"></i>
                Voltar</a>
        </form>
    </div>
    <!-- Footer-->
